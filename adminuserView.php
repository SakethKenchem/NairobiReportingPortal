...
function delete_user($conn, $userId)
{
    ...
    $sqlComments = "DELETE FROM comments WHERE userid = " . $userId;
    $conn->query($sqlComments);

    // Delete user's posts
    $sqlPosts = "DELETE FROM posts WHERE userid = " . $userId;
    $conn->query($sqlPosts);

    // Delete the user
    $sqlUser = "DELETE FROM userlogincredentials WHERE userid = " . $userId;
    $conn->query($sqlUser);
}
...
<head>
    ...
    <style>
        ...
        .greeting-container {
            ...
            /* Use the "slideLeftToRight" animation for the greeting */
            margin-top: 15px;
            margin-bottom: 19px;
            margin-left: 100px;
        }
                ...
                <?php foreach ($users as $user) : ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" onsubmit="return confirmBlockUnblock(<?php echo $user['is_blocked']; ?>);">
                            ...
                            <script>
                                function confirmBlockUnblock(isBlocked) {
                                    var action = (isBlocked == 1) ? 'unblock' : 'block';
